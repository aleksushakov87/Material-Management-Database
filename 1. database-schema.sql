...
/*
	MATERIAL MANAGEMENT SYSTEM – Database Initialization Script
	This script initializes the Material_Management_System database by:
	1. Dropping and recreating the database if it already exists.
	2. Creating all core entities including:
		- RoleTable, UserAccount, SUBCONTRACTOR, SUPPLIER
		- Transactional tables like FMR, MRN, PURCHASE_ORDER, MRR
		- Supporting tables like ITEM, INVENTORY, WAREHOUSE, LOCATION_
	3. Defining relationships through foreign keys to maintain referential integrity.
	4. Indexing key fields to optimize performance for queries and lookups.
    
	The design supports materials procurement, receiving, issuing, returning, and inventory management workflows 
    involving users, subcontractors, and suppliers.
*/

-- 1. Dropping database if exist
...
-- 3. CREATING FOREIGN KEYS for each entity
